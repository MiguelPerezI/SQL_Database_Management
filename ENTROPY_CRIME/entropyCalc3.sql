...
DECLARE @N INT;
SET @N = (SELECT COUNT(DISTINCT cod_expe) FROM Delitos);
PRINT N'-->Numero de conjuntos := ' + CONVERT(VARCHAR, @N);
--212230 suma de delitos ocurridos

DECLARE @tableA TABLE(cod_expe INT, Repeticion_Delito INT, Probabilidad FLOAT, logProb FLOAT);
INSERT @tableA(cod_expe, Repeticion_Delito, Probabilidad, logProb)
SELECT 	logProb.cod_expe,
	logProb.Repeticion_Delito,
	logProb.Probabilidad,
	LOG(logProb.Probabilidad, @N) AS logProb
FROM(
	SELECT  prob.cod_expe, 
		prob.Repeticion_Delito, 
		CONVERT(FLOAT, prob.Repeticion_Delito)/212230.0 AS Probabilidad
	FROM(
		SELECT cod_expe, COUNT(cod_expe) AS Repeticion_Delito
		FROM Delitos
		GROUP BY cod_expe
		ORDER BY COUNT(cod_expe) DESC OFFSET 0 ROWS
	) AS prob
...
SELECT *
FROM
(SELECT cod_expe, Repeticion_Delito, Probabilidad, logProb, Probabilidad*logProb AS plog_p FROM @tableA) AS Ent;
