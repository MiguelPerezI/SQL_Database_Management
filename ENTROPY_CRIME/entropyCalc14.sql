...
DECLARE @N INT;
SET @N = (SELECT COUNT(DISTINCT munocurr) FROM Delitos);
PRINT N'-->Numero de conjuntos := ' + CONVERT(VARCHAR, @N);
--212230 suma de delitos ocurridos

DECLARE @tableA TABLE(munocurr VARCHAR(10), Repeticion_Delito INT, Probabilidad FLOAT, logProb FLOAT);
INSERT @tableA(munocurr, Repeticion_Delito, Probabilidad, logProb)
SELECT 	logProb.munocurr,
	logProb.Repeticion_Delito,
	logProb.Probabilidad,
	LOG(logProb.Probabilidad, @N) AS logProb
FROM(
	SELECT  prob.munocurr, 
		prob.Repeticion_Delito, 
		CONVERT(FLOAT, prob.Repeticion_Delito)/212230.0 AS Probabilidad
	FROM(
		SELECT munocurr, COUNT(munocurr) AS Repeticion_Delito
		FROM Delitos
		GROUP BY munocurr
		ORDER BY COUNT(munocurr) DESC OFFSET 0 ROWS
	) AS prob
...
SELECT *
FROM
(SELECT munocurr, Repeticion_Delito, Probabilidad, logProb, Probabilidad*logProb AS plog_p FROM @tableA) AS Ent;
