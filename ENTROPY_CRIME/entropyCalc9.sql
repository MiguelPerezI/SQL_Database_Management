...
DECLARE @N INT;
SET @N = (SELECT COUNT(DISTINCT tipmodal) FROM Delitos);
PRINT N'-->Numero de conjuntos := ' + CONVERT(VARCHAR, @N);
--212230 suma de delitos ocurridos

DECLARE @tableA TABLE(tipmodal INT, Repeticion_Delito INT, Probabilidad FLOAT, logProb FLOAT);
INSERT @tableA(tipmodal, Repeticion_Delito, Probabilidad, logProb)
SELECT 	logProb.tipmodal,
	logProb.Repeticion_Delito,
	logProb.Probabilidad,
	LOG(logProb.Probabilidad, @N) AS logProb
FROM(
	SELECT  prob.tipmodal, 
		prob.Repeticion_Delito, 
		CONVERT(FLOAT, prob.Repeticion_Delito)/212230.0 AS Probabilidad
	FROM(
		SELECT tipmodal, COUNT(tipmodal) AS Repeticion_Delito
		FROM Delitos
		GROUP BY tipmodal
...
SELECT *
FROM
(SELECT tipmodal, Repeticion_Delito, Probabilidad, logProb, Probabilidad*logProb AS plog_p FROM @tableA) AS Ent;
