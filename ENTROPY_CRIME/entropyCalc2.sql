...
DECLARE @N INT;
SET @N = (SELECT COUNT(DISTINCT ubicgeob) FROM Delitos);
PRINT N'-->Numero de conjuntos := ' + CONVERT(VARCHAR, @N);
--212230 suma de delitos ocurridos

DECLARE @tableA TABLE(ubicgeob VARCHAR(10), Repeticion_Delito INT, Probabilidad FLOAT, logProb FLOAT);
INSERT @tableA(ubicgeob, Repeticion_Delito, Probabilidad, logProb)
SELECT 	logProb.ubicgeob,
	logProb.Repeticion_Delito,
	logProb.Probabilidad,
	LOG(logProb.Probabilidad, @N) AS logProb
FROM(
	SELECT  prob.ubicgeob, 
		prob.Repeticion_Delito, 
		CONVERT(FLOAT, prob.Repeticion_Delito)/212230.0 AS Probabilidad
	FROM(
		SELECT ubicgeob, COUNT(ubicgeob) AS Repeticion_Delito
		FROM Delitos
		GROUP BY ubicgeob
		ORDER BY COUNT(ubicgeob) DESC OFFSET 0 ROWS
	) AS prob
...
SELECT *
FROM
(SELECT ubicgeob, Repeticion_Delito, Probabilidad, logProb, Probabilidad*logProb AS plog_p FROM @tableA) AS Ent;
