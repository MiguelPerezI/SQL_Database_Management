...
DECLARE @N INT;
SET @N = (SELECT COUNT(DISTINCT gdoconsu) FROM Delitos);
PRINT N'-->Numero de conjuntos := ' + CONVERT(VARCHAR, @N);
--212230 suma de delitos ocurridos

DECLARE @tableA TABLE(gdoconsu INT, Repeticion_Delito INT, Probabilidad FLOAT, logProb FLOAT);
INSERT @tableA(gdoconsu, Repeticion_Delito, Probabilidad, logProb)
SELECT 	logProb.gdoconsu,
	logProb.Repeticion_Delito,
	logProb.Probabilidad,
	LOG(logProb.Probabilidad, @N) AS logProb
FROM(
	SELECT  prob.gdoconsu, 
		prob.Repeticion_Delito, 
		CONVERT(FLOAT, prob.Repeticion_Delito)/212230.0 AS Probabilidad
	FROM(
		SELECT gdoconsu, COUNT(gdoconsu) AS Repeticion_Delito
		FROM Delitos
		GROUP BY gdoconsu
		ORDER BY COUNT(gdoconsu) DESC OFFSET 0 ROWS
	) AS prob
...
SELECT *
FROM
(SELECT gdoconsu, Repeticion_Delito, Probabilidad, logProb, Probabilidad*logProb AS plog_p FROM @tableA) AS Ent;
