...
DECLARE @N INT;
SET @N = (SELECT COUNT(DISTINCT formcomi) FROM Delitos);
PRINT N'-->Numero de conjuntos := ' + CONVERT(VARCHAR, @N);
--212230 suma de delitos ocurridos

DECLARE @tableA TABLE(formcomi INT, Repeticion_Delito INT, Probabilidad FLOAT, logProb FLOAT);
INSERT @tableA(formcomi, Repeticion_Delito, Probabilidad, logProb)
SELECT 	logProb.formcomi,
	logProb.Repeticion_Delito,
	logProb.Probabilidad,
	LOG(logProb.Probabilidad, @N) AS logProb
FROM(
	SELECT  prob.formcomi, 
		prob.Repeticion_Delito, 
		CONVERT(FLOAT, prob.Repeticion_Delito)/212230.0 AS Probabilidad
	FROM(
		SELECT formcomi, COUNT(formcomi) AS Repeticion_Delito
		FROM Delitos
		GROUP BY formcomi
		ORDER BY COUNT(formcomi) DESC OFFSET 0 ROWS
	) AS prob
...
SELECT *
FROM
(SELECT formcomi, Repeticion_Delito, Probabilidad, logProb, Probabilidad*logProb AS plog_p FROM @tableA) AS Ent;
