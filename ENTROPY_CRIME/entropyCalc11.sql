...
DECLARE @N INT;
SET @N = (SELECT COUNT(DISTINCT ticoncur) FROM Delitos);
PRINT N'-->Numero de conjuntos := ' + CONVERT(VARCHAR, @N);
--212230 suma de delitos ocurridos

DECLARE @tableA TABLE(ticoncur INT, Repeticion_Delito INT, Probabilidad FLOAT, logProb FLOAT);
INSERT @tableA(ticoncur, Repeticion_Delito, Probabilidad, logProb)
SELECT 	logProb.ticoncur,
	logProb.Repeticion_Delito,
	logProb.Probabilidad,
	LOG(logProb.Probabilidad, @N) AS logProb
FROM(
	SELECT  prob.ticoncur, 
		prob.Repeticion_Delito, 
		CONVERT(FLOAT, prob.Repeticion_Delito)/212230.0 AS Probabilidad
	FROM(
		SELECT ticoncur, COUNT(ticoncur) AS Repeticion_Delito
		FROM Delitos
		GROUP BY ticoncur
		ORDER BY COUNT(ticoncur) DESC OFFSET 0 ROWS
	) AS prob
...
SELECT *
FROM
(SELECT ticoncur, Repeticion_Delito, Probabilidad, logProb, Probabilidad*logProb AS plog_p FROM @tableA) AS Ent;
